...
Building anatomical MRIQC workflow for files: {', '.join(dataset)}."""
    ...
    segment = pe.Node(
        fsl.FAST(segments=True, out_basename="segment"), name="segmentation", mem_gb=5
    )
    # 7. Compute IQMs
    iqmswf = compute_iqms()
    # Reports
...
def compute_iqms(name="ComputeIQMs"):
    ...
    outputnode = pe.Node(
        niu.IdentityInterface(fields=["out_file", "noisefit"]), name="outputnode"
    )
    ...
    datasink = pe.Node(
        IQMFileSink(
            out_dir=config.execution.output_dir, dataset=config.execution.dsname
        ),
        name="datasink",
...
def headmsk_wf(name="HeadMaskWorkflow"):
    ...
    if use_bet:
        ...
        enhance = pe.Node(
            niu.Function(
                input_names=["in_file"], output_names=["out_file"], function=_enhance
            ),
            name="Enhance",
        )
        estsnr = pe.Node(
        ...
        workflow.connect(
            [
                (inputnode, estsnr, [("in_file", "in_file"), ("in_segm", "seg_file")]),
...
def airmsk_wf(name="AirMaskWorkflow"):
    ...
    workflow.connect(
        [
            ...
            (inputnode, qi1, [("in_file", "in_file"), ("head_mask", "head_mask")]),
