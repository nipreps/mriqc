...
import numpy as np
import pandas as pd
from mriqc.classifier.data import get_bids_cols, read_dataset
...
class CVHelper(CVHelperBase):
    ...
    def setXtest(self, X, Y):
        self._Xtest, _ = read_dataset(
            X, Y, rate_label=self._rate_column, binarize=not self._multiclass
        )
    ...
    def fit(self):
        ...
        if self._nestedcv or self._nestedcv_kfold:
            ...
            LOG.info(
                "Nested CV [avg] %s=%.3f (+/-%.3f), accuracy=%.3f (+/-%.3f), "
                "recall=%.3f (+/-%.3f).",
                self._scorer,
                nested_means[0],
                nested_std[0],
        ...
        np.savez(
            os.path.abspath(self._gen_fname(suffix="cvres", ext="npz")),
        ...
        LOG.info(
            "CV [Best model] %s=%s, mean=%.3f, std=%.3f.",
            self._scorer,
            grid.best_score_,
    ...
    def evaluate(
        ...
        LOG.info(
            "Performance on evaluation set (%s)",
        ...
        if save_roc:
            plot_roc_curve(
                self._Xtest[[self._rate_column]].values.ravel(),
                prob_y,
        ...
        if self._permutation_test:
            ...
            permutation_scores = permutation_test_score(
                ...
                n_permutations=self._permutation_test,
                n_jobs=1,
            )
    ...
    def _save_pred_table(self, sample, prob_y, pred_y, suffix):
        ...
        if self._multiclass:
            probs = ["proba_%d" % i for i in list(range(prob_y.shape[1]))]
