...
if [[ $CIRCLE_NODE_INDEX -eq 0 ]]; then
	docker run -i -v /etc/localtime:/etc/localtime:ro-e TZ=PST --entrypoint="/usr/bin/run_tests" oesteban/mriqc
fi

i=0
for test_params in "${MRIQC_TESTS[@]}"; do
	if [ $(($i % $CIRCLE_NODE_TOTAL)) -eq $CIRCLE_NODE_INDEX ]; then
	    docker run -i -e TZ=PST -v /etc/localtime:/etc/localtime:ro -v ~/scratch:/scratch -w /scratch oesteban/mriqc ${test_params}
	fi
	((i=i+1))
done
